...
export function CategoryPage({ eyebrow, title, description, department, categories, image, products }: CategoryPageProps) {
  const displayProducts = products ?? byDepartment(department);
  return (
    <>
      ...
      <section className="py-20 md:py-28 max-w-7xl mx-auto px-6 md:px-8">
        <Reveal>
          <CatalogView products={displayProducts} categories={categories} />
        </Reveal>
      </section>
    </>
  );
